...
import "./Dropdown.css";
import { StatusIcon, RiskIcon } from "../../PloshadkiTable/Table/Icons";

export default function Dropdown({ item, data, func, filterType, isFiltersReset }) {
  ...
  useEffect(() => {
    const initialValues = searchParams.getAll(filterType).map(decodeURIComponent); // Получаем значения из URL
  
    // Проверяем, отличаются ли значения
    if (JSON.stringify(initialValues) !== JSON.stringify(selectedValues)) {
      setSelectedValues(initialValues); // Сохраняем значения в state
      func(filterType, initialValues); // Передаем значения в родительский компонент
    }
  }, [searchParams, filterType, func, selectedValues]);
  ...
  const toggleValue = (value) => {
    setSelectedValues((prev) => {
      ...
      const newValues = isSelected
        ? prev.filter((v) => v !== value) // Удаляем значение
        : [...prev, value]; // Добавляем значение

      func(filterType, newValues); // Передаем обновленные значения в родительский компонент
      return newValues;
    });
  };
  ...
  return (
    <div className="flex items-center">
      <Button item={item} handleDropdownClick={handleDropdownClick} />
      <div
        ref={container}
        className={`fixed bg-white transition-transform duration-300 ease-in-out z-50 ${
          dropdownState ? "dropdown-open" : "dropdown-closed"
        }`}
        style={{ top: "5%" }}
      >
                    ...
                    >
                      <div
                        className={`border-primary mr-2 flex h-4 w-4 items-center justify-center rounded-sm border ${
                          isSelected(value) ? "bg-primary text-primary-foreground" : "opacity-60 [&_svg]:invisible"
                        }`}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                      ...
                      <RiskIcon risk={value} mini={true} />
                      <StatusIcon status={value} mini={true} />
                      <span>{value}</span>
                    </div>
                  ))}
                </div>
