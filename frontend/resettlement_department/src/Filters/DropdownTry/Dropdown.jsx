...
export default function Dropdown({ item, data, func, filterType, isFiltersReset }) {
    ...
    // Синхронизация состояния с параметрами URL
    useEffect(() => {
        const initialValues = searchParams.getAll(filterType).map(decodeURIComponent);

        // Обновляем состояние только если оно отличается от текущего состояния
    ...
    useEffect(() => {
        if (!isFiltersReset) {
            setSelectedValues([]); // Сбрасываем выбранные значения
        }
    }, [isFiltersReset]);
