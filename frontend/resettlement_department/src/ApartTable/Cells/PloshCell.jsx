    ...
    <div className="flex w-full flex-row items-center justify-start gap-1 text-gray-400">
      <div className="flex flex-1 flex-col items-start justify-start truncate">
        ...
        <div className="text-xs">
          <span className='font-bold'>{value['room_count']}</span> комн. {`${value['type_of_settlement'] ? value['type_of_settlement'] === 'частная собственность,' ? 'ЧС,' : value['type_of_settlement'] : '-'} ${value['floor'] ? value['floor'] + ' этаж' : "-"}`}
        </div>
      </div>
    </div>
  );
