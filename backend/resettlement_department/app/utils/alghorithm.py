        ...
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                df_old_apart = pd.DataFrame(old_aparts, columns=[
                    'family_apartment_needs_id', 'kpu_number', 'district', 'municipal_district', 'room_count', 
                    'full_living_area', 'total_living_area', 'living_area', 'is_special_needs_marker', 
                    'min_floor', 'max_floor', 'buying_date', 'ages', 'members_amount', 
                    'oldest', 'is_queue', 'queue_square'
                ])
                
                df_new_apart = pd.DataFrame(new_aparts, columns=[
                    'new_apart_id', 'district', 'municipal_district', 'house_address', 'apart_number',
                    'floor', 'room_count', 'full_living_area', 'total_living_area', 'living_area',
                ...
                df_old_apart['combined_area'] = df_old_apart['combined_area'].fillna(0)  # или другой подходящий метод
                ...
                cursor.executemany('''UPDATE public.family_apartment_needs
                                    SET rank = %s
                                    WHERE family_apartment_needs_id = %s''', old_apart_rank_update)
                conn.commit()
                cursor.executemany('''UPDATE public.new_apart
                                    SET rank = %s
                ...
                if not date:
                    ...
                    for record in history_data:
                        history_id, old_house_addresses, new_house_addresses, status_id, created_at, updated_at = record
                        if old_house_addresses == old_selected_addresses and new_house_addresses == new_selected_addresses:
                            record_exists = True
                            break
                else:
                    cursor.execute("SELECT DISTINCT house_address FROM public.family_structure WHERE affair_id IN (SELECT affair_id FROM public.family_apartment_needs WHERE created_at = (SELECT MAX(created_at) FROM public.family_apartment_needs))")
                    old_selected_addresses = [r[0] for r in cursor.fetchall()]
                    cursor.execute("SELECT DISTINCT house_address FROM public.new_apart WHERE created_at = (SELECT MAX(created_at) FROM public.new_apart)")
                ...
                for old_apart_id, new_apart_id, insert_date in offers_to_insert:
                    cursor.execute(
                        """
                        INSERT INTO public.offer (family_apartment_needs_id, new_apart_id, created_at, status_id) 
                        VALUES (%s, %s, %s, 7)
                ...
                if cannot_offer_to_insert:
                    ...
                    cursor.executemany(
                        """
                        INSERT INTO public.cannot_offer (family_apartment_needs_id, insert_date) 
                        VALUES (%s, %s)
                        ON CONFLICT (family_apartment_needs_id) 
                        DO UPDATE SET 
