...
class DashboardService:
    ...
    def get_dashboard_data(self):
        ...
        for new_building_id, _, _, _, old_buildings in dashboard_data:
            ...
            if all_done:
                ...
                categories["halfInProgress"][new_building_id] = list(
                    old_buildings.keys()
                )
        ...
        for risk_type, buildings in data1.items():
            for building_id, details in buildings.items():
                ...
                if isinstance(details, dict) and "date" in details:
                    ...
                    raise ValueError(
                        f"Unexpected data format for building {building_id}: {details}"
                    )

                for i, (low, high) in enumerate(
                    zip(category_limits[:-1], category_limits[1:])
                ):
                    if low <= months < high:
    ...
    @cached(ttl=1800, serializer=PickleSerializer())  # 300 секунд = 5 минут
    async def get_district_and_municipal_district_attempt(self):
        result = await self.db.get_district_and_municipal_district_attempt()
        dash = {}
        if result is not None:
            for row in result:
                ...
                if district not in dash:
                    dash[district] = {}
                if municipal_district not in dash[district].keys():
                    dash[district][municipal_district] = {} 
                dash[district][municipal_district][attempt] = {
                        "Согласие": approve,
                        "Суд": court,
                        "Отказ (Выход из суда)": exit_from_court,
                        "Отказ": refusal,
                        "Ожидание": waiting,
                        "Ждет одобрения": waiting_for_approve,
                        "Всего": all,
                    }
        return dash
    ...
    async def get_house_address_with_attempt(self):
        result = await self.db.get_house_address_with_attempt()
        dash = {}
        if result is not None:
            for row in result:
                (
                ...
                if house_address not in dash:
                    dash[house_address] = {}
                dash[house_address][attempt] = {
                        "Согласие": approve,
                        "Суд": court,
                        "Отказ (Выход из суда)": exit_from_court,
                        "Отказ": refusal,
    ...
    async def get_district_with_attempt(self):
        result = await self.db.get_district_with_attempt()
        dash = {}
        if result is not None:
            for row in result:
                (
