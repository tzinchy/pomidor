...
def generate_excel_from_two_dataframes(output_path=None, file_path=None, new_selected_addresses=None, old_selected_addresses=None,
    ...
    query = """
        SELECT ol.house_address, ol.apart_number, ol.type_of_settlement, ol.kpu_number, new_apart_id, 
               ...
               n.living_area, n.floor, cin_address, cin_schedule, dep_schedule, phone_osmotr, phone_otvet,  ol.unom, start_date, cin.otdel
        FROM recommendation.offer o
        JOIN recommendation.old_apart ol USING (old_apart_id)
        JOIN recommendation.new_apart n USING (new_apart_id)
		join recommendation.cin on cin.old_address = ol.house_adress
        WHERE ol.ochered <> 1
    """
    ...
    if old_selected_districts:
        query += " AND ol.district IN %s"
        params.append(tuple(old_selected_districts))
    if old_selected_areas:
        query += " AND ol.municipal_district IN %s"
        params.append(tuple(old_selected_areas))
    if date:
        query += " AND ol.created_at = (SELECT MAX(created_at) FROM family_apartment_needs)"
    ...
    if new_selected_districts:
        query += " AND n.district IN %s"
        params.append(tuple(new_selected_districts))
    if new_selected_areas:
        query += " AND n.municipal_district IN %s"
        params.append(tuple(new_selected_areas))
    if date:
        query += " AND n.created_at = (SELECT MAX(created_at) FROM new_apart)"
...
Просим довести указанную в письме информацию до всех правообладателей.""",
    ...
    wb.save(output_path)
    print(f"Excel файл '{file_path}' создан.")
    conn.close()