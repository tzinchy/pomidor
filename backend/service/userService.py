...
class UserService:
    ...
    async def validate_user(cls, email: EmailStr, password: str):
        ...
        """
        try:
            user = await UserDAO.find_user_by_email(email=email)
            if not user:
                logger.warning(f"User with email {email} not found.")
                raise UserNotFoundException()  # Raise a UserNotFoundException if no user is found

            password_from_db = await UserDAO.find_password_by_email(email=email)
            if cls.validate_password(password_provided=password, password_from_db=password_from_db):
    ...
    async def create_jwt_token(cls, email: EmailStr):
        ...
        """
        user_data = await UserDAO.find_user_info_for_jwt(email=email)
        if user_data:
    ...
    async def register_user(cls, email: EmailStr, password: str):
        ...
        existing_user = await UserDAO.find_user_by_email(email=email)
        if existing_user:
            logger.warning(f"User with email {email} already exists.")
            raise UserAlreadyExistsException()  # Raise exception if user already exists

        # Create the new user
        await UserDAO.create_user(email=email, password=password)
