...
class ApartmentStatuses(Base):
    ...
    full_name = Column(String(225), computed="((group_name || ' - '::text) || status_name)", persisted=True)
...
class BuildingsOld(Base):
    ...
    okrug_order = Column(Integer, computed="""
        CASE
            WHEN (okrug = 'ЦАО') THEN 1
            WHEN (okrug = 'САО') THEN 2
            WHEN (okrug = 'СВАО') THEN 3
            WHEN (okrug = 'ВАО') THEN 4
            WHEN (okrug = 'ЮВАО') THEN 5
            WHEN (okrug = 'ЮАО') THEN 6
            WHEN (okrug = 'ЮЗАО') THEN 7
            WHEN (okrug = 'ЗАО') THEN 8
            WHEN (okrug = 'СЗАО') THEN 9
            WHEN (okrug = 'ЗелАО') THEN 10
            WHEN (okrug = 'ТАО') THEN 11
            WHEN (okrug = 'НАО') THEN 12
            ELSE 13
        END
    """, persisted=True)
    status_order = Column(Integer, computed="""
        CASE
            WHEN ((terms -> 'actual'::text) ->> 'demolitionEnd'::text) IS NOT NULL) THEN 4
            WHEN ((terms -> 'actual'::text) ->> 'secontResetlementEnd'::text) IS NOT NULL) THEN 3
            WHEN ((terms -> 'actual'::text) ->> 'firstResetlementEnd'::text) IS NOT NULL) THEN 2
            WHEN ((terms -> 'actual'::text) ->> 'firstResetlementStart'::text) IS NOT NULL) THEN 1
            ELSE 0
        END
    """, persisted=True)
...
class ApartmentLitigationsTemp(Base):
    ...
    case_category_id = Column(String(25), computed="substring((case_category::text), '[^ ]+'::text)", persisted=True)
    version = Column(Integer, nullable=False, default=1)
...
class ApartmentLitigations(Base):
    ...
    final_result = Column(String(225), computed="""
        CASE
            WHEN (((case_result ~~ '%тказан%'::text) OR (case_result ~~ '%довлетворен%'::text)) AND (hearing_result_class = 'В пользу ДГИ'::text)) THEN 'Выиграно'::text
            WHEN (((case_result ~~ '%тказан%'::text) OR (case_result ~~ '%довлетворен%'::text)) AND (hearing_result_class = 'Не в пользу ДГИ'::text)) THEN 'Проиграно'::text
            WHEN (hearing_result_class !~~ '%ДГИ%'::text) THEN 'Не по существу'::text
            ELSE 'Ведётся'::text
        END
    """, persisted=True)
    version = Column(Integer, nullable=False, default=1)
...
class ApartmentLitigationHearings(Base):
    ...
    appeal_date = Column(Date)
    priority = Column(Integer, computed="""
        CASE
            WHEN (((hearing_result_class ~~ '%ДГИ%'::text) AND (subject_of_proceedings = 'Основное дело'::text)) THEN 2
            WHEN (subject_of_proceedings = 'Основное дело'::text) THEN 1
            ELSE 0
        END
    """, persisted=True)
    version = Column(Integer, nullable=False, default=1)
...
class ApartmentConnections(Base):
    ...
    status = Column(String(225), computed="""
        CASE
            WHEN (order_reason = 'Московский фонд реновации жилой застройки'::text) THEN 'ФР'::text
            WHEN (contract_date IS NOT NULL) THEN 'Договор'::text
            WHEN (contract_status IS NOT NULL) THEN 'Проект договора'::text
            WHEN (rd_date IS NOT NULL) THEN 'Распоряжение'::text
            WHEN ((inspection_response)::text ~~ '%Отказ%'::text) THEN 'Отказ'::text
            WHEN ((inspection_response)::text ~~ '%Согласие%'::text) THEN 'Согласие'::text
            WHEN (inspection_date IS NOT NULL) THEN 'Осмотр'::text
            ELSE 'В работе'::text
        END
    """, persisted=True)
    status_prio = Column(Integer, computed="""
        CASE
            WHEN (order_reason = 'Московский фонд реновации жилой застройки'::text) THEN 6
            WHEN (contract_date IS NOT NULL) THEN 6
            WHEN (contract_status IS NOT NULL) THEN 5
            WHEN (rd_date IS NOT NULL) THEN 4
            WHEN ((inspection_response)::text ~~ '%Отказ%'::text) THEN 2
            WHEN ((inspection_response)::text ~~ '%Согласие%'::text) THEN 3
            WHEN (inspection_date IS NOT NULL) THEN 1
            ELSE 0
        END
    """, persisted=True)
...
class Users(Base):
    ...
    id = Column(Integer, primary_key=True, server_default=FetchedValue())
    login = Column(String(225), nullable=False, unique=True)
    fio = Column(String(225))
    password = Column(String, nullable=False)
    refresh_token_version = Column(Integer, default=1)
    roles = Column(ARRAY(String), default=['user'])
    control_data = Column(JSON, server_default='{"roles": [], "priority": 0, "approveTo": []}')
    control_settings = Column(JSON, server_default='{}')
    token = Column(UUID, server_default='gen_random_uuid()')
...
class Messages(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default='now()')
    updated_at = Column(DateTime(timezone=True), server_default='now()')
    author_id = Column(Integer, ForeignKey('renovation.users.id'))
    building_id = Column(Integer, ForeignKey('renovation.buildings_old.id'))
    apartment_id = Column(Integer, ForeignKey('renovation.apartments_old_temp.id'))
    message_type = Column(String(225), nullable=False, server_default='comment')
    message_content = Column(Text)
    valid_until = Column(Date)
    is_deleted = Column(Boolean, default=False)
    needs_answer = Column(Boolean, default=False)
    answer_date = Column(Date)
    message_payload = Column(JSON, server_default='[]')
    
    author = relationship("Users")
    building = relationship("BuildingsOld")
    apartment = relationship("ApartmentsOldTemp")
